...
import {
  CategoryDonut,
  MonthlyBarChart,
} from "@/components/charts/insights-charts-dynamic";
...
export function InsightsClient({
  ...
  const monthTotal = useMemo(
    () => sumOccurrences(selectedMonthOccurrences, defaultCurrency),
    [selectedMonthOccurrences, defaultCurrency],
  );

  const categoryData = useMemo(() => {
    const grouped = groupByCategory(selectedMonthOccurrences);
    return Array.from(grouped.values()).map(({ category, total }) => ({
      name: category?.name ?? t("uncategorized"),
      value: total,
      color: category?.color,
    }));
  }, [selectedMonthOccurrences, t]);
  ...
  function setLedgerFilter(nextLedger: LedgerFilter) {
    ...
    syncInsightsUrl(monthKey, nextLedger);
  }
      ...
      <PageTransition className="space-y-8">
        <header className="flex items-start justify-between gap-3">
          ...
          <div className="flex shrink-0 items-center gap-1">
            <PrivacyToggle />
            <LedgerFilterSwitcher value={ledger} onChange={setLedgerFilter} className="mt-1" />
          </div>
        </header>
